...
if exist "%PF_BIN%run.properties" (
    ...
    for /F "tokens=1,2 delims==" %%G in ('type "%PF_BIN%run.properties"') do (set %%G=%%H)
) ELSE (
	echo Missing %PF_HOME%\bin\run.properties; using defaults.
)

if "%org.bouncycastle.fips.approved_only%"=="" (
    set PF_JAVA_OPTS=%PF_JAVA_OPTS% -Dorg.bouncycastle.fips.approved_only=true
) else (
    set PF_JAVA_OPTS=%PF_JAVA_OPTS% -Dorg.bouncycastle.fips.approved_only=%org.bouncycastle.fips.approved_only%
)
